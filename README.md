...
Бот сам собирает спортивные события на неделю (сейчас — тестовые данные в `sports_events.py`), показывает администратору отфильтрованную афишу, по кнопкам генерирует посты через **Google Gemini** и может опубликовать их в группу Gastrobar. Расписание: еженедельная рассылка по понедельникам 11:00 и ежедневная проверка в 12:00 (**Asia/Ho_Chi_Minh**).
